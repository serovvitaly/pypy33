...
result = c.execute("SELECT ow.id, ow.source_url, ps.name FROM owners ow JOIN parsers ps ON ow.parser_id = ps.id WHERE ow.nic IS NULL LIMIT 10")
...
for row in result:
    ...
    c.execute("UPDATE owners SET nic=:nic WHERE id=" + str(owner_id), parser_obj.__dict__)
    conn.commit()
